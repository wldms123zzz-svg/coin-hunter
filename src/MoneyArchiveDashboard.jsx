...
const PastelFront = ({ coin, size }) => {
  ...
  return (
    <svg width={s} height={s} viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      {/* 안쪽 링 */}
      <circle cx="100" cy="100" r="74" fill="none" stroke={coin.pastelDark} strokeWidth="1.5" opacity="0.35"/> ㄴ
      {/* 앞면 아이콘 */}
      ...
      <text x="100" y="130" textAnchor="middle"
        ...
        fill={coin.gradeColor} opacity="0.9"
        letterSpacing="-1">{coin.denom}</text>
      {/* 단위 */}
      <text x="100" y="158" textAnchor="middle"
        fontSize="13" fontWeight="600" letterSpacing="2"
        fontFamily="'Pretendard',system-ui,sans-serif"
        fill={coin.gradeColor} opacity="0.5">원</text>
    </svg>
  );
};
  ...
  return (
    <svg width={s} height={s} viewBox="0 0 200 200" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="100" y="162" textAnchor="middle"
        ...
        fontFamily="'Pretendard',system-ui,sans-serif"
        fill={coin.gradeColor} opacity="0.5">{coin.year}</text>
    </svg>
  );
};
...
const GridCoin = ({ coin, found }) => (
  <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:5, padding:"12px 4px",
    ...
    {found
      ...
      : <span style={{ fontSize:13, color:"#ffe419" }}>🔒</span>}
  </div>
);
